...
### vstart & org
org本身不能决定程序被加载到什么位置，它只是告诉编译器，我的程序将来会被加载到某个位置，请编译器在链接时按照org给出的值，调整数据引用的地址(org值+程序内数据引用偏移地址)；
比如`org 0x7c00`，BIOS会将mbr程序加载到`0x7c00`位置，那么在mbr中，为了正确寻址，就要指定这个偏移量。
